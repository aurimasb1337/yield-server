...
const bunniLensAddress = '0x3cEb26bb6AD94F2dFdD98f10cB4D6CAF02bec9Dc'; //base only, for now
const baseUrl = 'https://api.studio.thegraph.com';
const chains = {
  base: `${baseUrl}/query/59130/v3alb/version/latest`,
...
const topLvl = async (
  ...
  try {
    ...
    console.log('bunniLensCalls', bunniLensCalls);

    let bunniVaultData = await sdk.api.abi.multiCall({
      abi: bunniLensV2Abi.find((m) => m.name === 'getBunniVaults'),
      calls: bunniLensCalls,
      chain: chainString,
      permitFailure: true,
    });
    bunniVaultData = bunniVaultData?.output;
    // console.log('bunniVaultData output', bunniVaultData.output);
    ...
    const vaultDetails = await sdk.api.abi.multiCall({
      abi: bunniLensV2Abi.find((m) => m.name === 'pricePerFullShare'),
